...
/**
 * mong 데이터소스 초기 데이터 적재.
 *
 * 스크립트는 configs 서브모듈에서 온다:
 *   configs/properties/adapter-out/mong-persistence-adapter-out/*.sql
 ...
 * 여기서는 그 방법이 통하지 않는다 — battle · mong · task 세 EntityManager 가 모두
 * {@code com.monglife.module.common.jpa.entity} 를 스캔해서 같은 DB 의
 * {@code monglife_comn_code} · {@code monglife_group_code} 를 각자 만든다.
 * local 은 {@code create-drop} 이라, EMF 하나에만 순서를 걸면 적재가 끝난 뒤에 생성되는
 * 다른 EMF 가 그 테이블을 drop 해 시드를 날려 버린다. 그래서
 * {@link SmartInitializingSingleton} 으로 <b>모든 싱글턴이 만들어진 뒤</b>에 돈다.
 */
@Slf4j
...
public class SqlInitConfig implements SmartInitializingSingleton {
     ...
     */
    static final String CONFIG_NAME            = "mongSqlInitConfig";
